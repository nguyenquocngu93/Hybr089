...
var KEYWORDS = ['manifest.json','stream','configure','api','play'];
...
function buildManifest(cfg, configStr, pub) {
    return {
        id: 'com.hybrid.addon', version: '6.3.0', name: '🎬 Hybrid Addon',
        description: 'Torrentio + jac.red → TorrServer | Made with love ❤️',
...
function searchJacred(imdbId, type, maxResults, jacSort, apiUrl) {
    return getRuTitleFromTMDb(imdbId, type).then(function(ruTitle) {
        ...
        return Promise.all(promises).then(function() {
            ...
            unique.sort(function(a, b) { if (jacSort === 'seeds') return b.sid - a.sid; if (jacSort === 'date') return b.date - a.date; return b.sizeGB - a.sizeGB; });
...
function handleStream(type, id, cfg, res, pub) {
    ...
    var streams = [], completed = 0, total = (cfg.jacredEnabled ? 1 : 0) + (cfg.torrentioEnabled ? 1 : 0);
    ...
    if (cfg.jacredEnabled) {
        ...
        searchJacred(imdbId, type, cfg.maxResults || 30, jacSort, apiUrl).then(function(results) {
...
function buildConfigPage(cfg, configStr, pub) {
    ...
    return '<!DOCTYPE html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Hybrid Addon</title><style>' + css + '</style></head><body><div class="wrap"><h1>🎬 Hybrid Addon</h1><div class="card"><h2>📋 Link Config Torrentio</h2><div class="fg"><label>Dán link config</label><textarea id="configLink" placeholder="https://torrentio.strem.fun/..."></textarea></div><button class="btn" style="width:100%;margin-bottom:8px" onclick="applyTorrentioConfig()">⚡ Áp dụng</button><button class="btn" style="width:100%" onclick="resetTorrentioConfig()">🔄 Reset</button></div><div class="card"><h2>🟢 Torrentio</h2><div class="trow"><div class="ti"><span>🟢 Bật Torrentio</span></div><label class="sw"><input type="checkbox" id="torrentioEnabled"' + (cfg.torrentioEnabled ? ' checked' : '') + '><span class="sl"></span></label></div></div><div class="card"><h2>🖥️ TorrServer</h2><div class="fg"><label>URL</label><input type="text" id="tsUrl" value="' + (cfg.torrServerUrl || '') + '" placeholder="http://192.168.1.100:8090"><p class="hint" style="color:#aaa;font-size:13px;margin-top:4px"><strong>🖥️ TorrServer</strong> - Xem torrent online không cần tải. <a href="https://github.com/YouROK/TorrServer" target="_blank" style="color:#667eea;">Tải TorrServer</a> · Cần điền đúng IP:Port</p></div></div><div class="card"><h2>🔴 jac.red Settings</h2><div class="info-box"><p><strong>📡 jac.red</strong> - Công cụ tìm kiếm torrent Nga, hỗ trợ tiếng Nga, tìm phim lẻ và phim bộ chất lượng cao từ các tracker: RuTracker, KinoZal, NNMClub, Toloka, RuTor...</p><p><strong>🌐 Domain:</strong> Nếu một domain không truy cập được, hãy chọn domain khác.</p></div><div class="trow"><div class="ti"><span>🔴 Bật jac.red</span></div><label class="sw"><input type="checkbox" id="jacredEnabled"' + (cfg.jacredEnabled ? ' checked' : '') + '><span class="sl"></span></label></div><div class="fg"><label>Domain jac.red</label><select id="jacredDomain">' + domainOptions + '</select><p class="hint" style="color:#555;font-size:11px;margin-top:4px">🔥 jac.red, jac-red.ru, jr.maxvol.pro, ru.jacred.pro, jacred.stream</p></div><div class="fg"><label>Sắp xếp</label><div class="sort-grid"><div class="sort-btn' + (jacredSort === 'size' ? ' active' : '') + '" onclick="setJacSort(\'size\',this)">💾 Dung Lượng</div><div class="sort-btn' + (jacredSort === 'seeds' ? ' active' : '') + '" onclick="setJacSort(\'seeds\',this)">👥 Seeds</div><div class="sort-btn' + (jacredSort === 'date' ? ' active' : '') + '" onclick="setJacSort(\'date\',this)">📅 Mới Nhất</div></div><input type="hidden" id="jacredSort" value="' + jacredSort + '"></div><div class="fg"><label>Số kết quả</label><input type="number" id="maxResults" value="' + (cfg.maxResults || 30) + '" min="5" max="100"></div></div><div class="card"><h2>🎌 Anime Mode</h2><div class="info-box"><p><strong>🎌 Anime Mode</strong> - Dành cho pack anime bị TorrServer parse sai metadata. Sẽ lọc file <500MB, bỏ qua OP/ED, và đếm file theo thứ tự.</p></div><div class="trow"><div class="ti"><span>🎌 Bật Anime Mode</span><small>Bật khi xem anime bị chọn sai tập</small></div><label class="sw"><input type="checkbox" id="animeMode"' + (cfg.animeMode ? ' checked' : '') + '><span class="sl"></span></label></div></div><div class="card"><h2>📦 Link Cài Đặt</h2><div class="ubox" id="iurl">' + installUrl + '</div><div class="brow"><button class="btn" onclick="copyUrl()" style="flex:1">📋 Copy</button><a class="btn btn-p" href="' + stremioUrl + '" id="slink" style="flex:2">▶ Cài Vào Stremio</a></div></div><div class="card"><h2>📖 Mô Tả & GitHub</h2><div class="fg"><label>Dự án</label><p style="color:#aaa;font-size:13px;margin-bottom:10px">🎬 Hybrid Addon - Tổng hợp nguồn phim từ Torrentio và jac.red, phát qua TorrServer. Hỗ trợ phim lẻ và phim bộ với chọn tập thông minh.</p><a href="' + githubLink + '" target="_blank" class="btn" style="display:inline-flex;width:auto;background:#24243e">🐙 GitHub Repository</a></div></div><button class="btn btn-b" onclick="gen()">🔗 Tạo Link</button><div class="guide"><h3>📖 Hướng Dẫn Sử Dụng</h3><div class="step"><strong>1. Cấu hình Addon</strong><br/>- Nhập URL TorrServer<br/>- Bật/tắt nguồn jac.red và Torrentio<br/>- Chọn domain jac.red (nếu domain chính bị chặn)<br/>- Chọn cách sắp xếp kết quả (dung lượng, seeds, mới nhất)<br/>- Có thể dán link config Torrentio để tùy chỉnh nguồn</div><div class="step"><strong>2. Anime Mode</strong><br/>- Bật khi xem anime bị chọn sai tập<br/>- Tự động lọc file <500MB, bỏ OP/ED, đếm file theo thứ tự</div><div class="step"><strong>3. Tạo link & Cài đặt</strong><br/>- Nhấn "Tạo Link" sau khi cấu hình<br/>- Copy link hoặc nhấn "Cài Vào Stremio"<br/>- Addon sẽ hiển thị trong Stremio</div><div class="step"><strong>4. Sử dụng</strong><br/>- Tìm phim bất kỳ trong Stremio<br/>- Chọn stream từ "Hybrid Addon"<br/>- Với phim bộ, addon tự động chọn đúng tập</div><div class="step"><strong>⚠️ Lưu ý</strong><br/>- Cần có kết nối mạng ổn định<br/>- TorrServer nên đặt gần để giảm độ trễ<br/>- Một số phim cần thời gian để TorrServer tải metadata</div></div><div class="footer">Made with <i>❤️</i> love | v6.3.0</div></div><script>var currentConfig=' + JSON.stringify({ providers: cfg.providers, sortBy: cfg.sortBy, language: cfg.language, qualityfilter: cfg.qualityfilter }) + ';function setJacSort(v,el){document.getElementById("jacredSort").value=v;document.querySelectorAll(".sort-btn").forEach(b=>b.classList.remove("active"));el.classList.add("active")}function enc(o){return btoa(unescape(encodeURIComponent(JSON.stringify(o)))).replace(/\\+/g,"-").replace(/\\//g,"_").replace(/=/g,"")}function parseTorrentioLink(link){try{var u=new URL(link.replace("stremio://","https://"));var m=u.pathname.match(/\\/([^\\/]+)\\/manifest\\.json/);if(!m)return null;var p=m[1].split("|");var c={providers:[],sortBy:"size",language:"",qualityfilter:[]};p.forEach(x=>{var[k,v]=x.split("=");if(k==="providers")c.providers=v.split(",");else if(k==="sort")c.sortBy=v;else if(k==="language")c.language=v;else if(k==="qualityfilter")c.qualityfilter=v.split(",")});return c}catch(e){return null}}function applyTorrentioConfig(){var l=document.getElementById("configLink").value.trim();if(!l)return alert("Dán link!");var c=parseTorrentioLink(l);if(!c)return alert("Link lỗi!");currentConfig=c;gen()}function resetTorrentioConfig(){currentConfig=' + JSON.stringify(DEFAULT_TORRENTIO_CONFIG) + ';document.getElementById("configLink").value="";gen()}function getCurrentConfig(){return Object.assign({torrServerUrl:document.getElementById("tsUrl").value.trim(),jacredEnabled:document.getElementById("jacredEnabled").checked,torrentioEnabled:document.getElementById("torrentioEnabled").checked,jacredSortBy:document.getElementById("jacredSort").value,maxResults:parseInt(document.getElementById("maxResults").value)||30,jacredDomain:document.getElementById("jacredDomain").value,animeMode:document.getElementById("animeMode").checked},currentConfig)}function copyUrl(){var url=document.getElementById("iurl").textContent;if(navigator.clipboard&&navigator.clipboard.writeText){navigator.clipboard.writeText(url).then(function(){alert("✅ Đã copy link thành công!");}).catch(function(){fallbackCopy(url);});}else{fallbackCopy(url);}}function fallbackCopy(text){var textarea=document.createElement("textarea");textarea.value=text;textarea.style.position="fixed";textarea.style.top="-9999px";textarea.style.left="-9999px";document.body.appendChild(textarea);textarea.select();try{document.execCommand("copy");alert("✅ Đã copy link thành công!");}catch(err){alert("❌ Copy thất bại, vui lòng copy thủ công: "+text);}document.body.removeChild(textarea);}function gen(){var c=getCurrentConfig();var e=enc(c);var u=location.protocol+"//"+location.host+"/"+e+"/manifest.json";document.getElementById("iurl").textContent=u;document.getElementById("slink").href="stremio://"+u.replace(/^https?:\\/\\//,"")}</script></body></html>';
}
...
server.listen(PORT, '0.0.0.0', function() {
    console.log('\n✅ Hybrid Addon v6.3.0: http://localhost:' + PORT);

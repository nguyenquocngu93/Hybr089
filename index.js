...
function buildConfigPage(cfg, configStr, pub) {
 ...
 + '.url-box{background:rgba(0,0,0,0.4);border:1px solid rgba(255,255,255,0.15);border-radius:16px;padding:14px 16px;font-family:"SF Mono","Fira Code",monospace;font-size:12px;color:#93c5fd;word-break:break-all;margin:0;cursor:pointer;transition:all 0.3s;backdrop-filter:blur(10px);line-height:1.55}'
 + '.url-box:hover{background:rgba(96,165,250,0.1);border-color:#60a5fa;box-shadow:0 10px 30px rgba(96,165,250,0.14)}'
 ...
 + '.install-shell{position:relative;overflow:visible;background:linear-gradient(135deg,rgba(15,23,42,0.95),rgba(30,41,59,0.84));border:1px solid rgba(255,255,255,0.14);box-shadow:0 18px 48px rgba(0,0,0,0.36),inset 0 1px 0 rgba(255,255,255,0.09)}'
 + '.install-shell::after{content:"";position:absolute;inset:0;background:radial-gradient(circle at top right,rgba(96,165,250,0.18),transparent 34%),radial-gradient(circle at bottom left,rgba(167,139,250,0.16),transparent 36%);pointer-events:none}'
 + '.install-top{position:relative;z-index:1;display:flex;align-items:flex-start;justify-content:space-between;gap:16px;margin-bottom:16px}'
 + '.install-copy{flex:1;min-width:0}'
 + '.install-kicker{font-size:11px;letter-spacing:1.7px;text-transform:uppercase;color:rgba(255,255,255,0.52);margin-bottom:8px;font-weight:700}'
 + '.install-title{font-size:22px;font-weight:800;line-height:1.1;color:#fff;margin-bottom:8px}'
 + '.install-sub{font-size:13px;line-height:1.55;color:rgba(226,232,240,0.72)}'
 + '.menu-wrap{position:relative;z-index:2;flex-shrink:0}'
 + '.hamburger-btn{width:46px;height:46px;border:none;border-radius:999px;background:rgba(255,255,255,0.12);border:1px solid rgba(255,255,255,0.16);display:flex;align-items:center;justify-content:center;cursor:pointer;box-shadow:0 10px 22px rgba(0,0,0,0.22);transition:transform .22s,background .22s,border-color .22s,box-shadow .22s}'
 + '.hamburger-btn:hover{transform:translateY(-2px);background:rgba(255,255,255,0.16);border-color:rgba(148,163,184,0.35);box-shadow:0 14px 28px rgba(0,0,0,0.28)}'
 + '.hamburger-btn:active{transform:translateY(0)}'
 + '.hamburger-btn span{display:block;width:18px;height:2.4px;border-radius:999px;background:rgba(241,245,249,0.95);transition:transform .22s,opacity .22s}'
 + '.hamburger-btn span+span{margin-top:4.5px}'
 + '.menu-wrap.open .hamburger-btn{background:rgba(59,130,246,0.22);border-color:rgba(59,130,246,0.32)}'
 + '.menu-wrap.open .hamburger-btn span:nth-child(1){transform:translateY(6.9px) rotate(45deg)}'
 + '.menu-wrap.open .hamburger-btn span:nth-child(2){opacity:0}'
 + '.menu-wrap.open .hamburger-btn span:nth-child(3){transform:translateY(-6.9px) rotate(-45deg)}'
 + '.hamburger-menu{position:absolute;top:54px;right:0;min-width:228px;padding:10px;background:rgba(15,23,42,0.96);border:1px solid rgba(255,255,255,0.14);border-radius:18px;box-shadow:0 20px 48px rgba(0,0,0,0.38);backdrop-filter:blur(22px);opacity:0;transform:translateY(-8px) scale(.985);pointer-events:none;transition:opacity .18s,transform .18s}'
 + '.menu-wrap.open .hamburger-menu{opacity:1;transform:translateY(0) scale(1);pointer-events:auto}'
 + '.menu-item{display:flex;align-items:center;gap:12px;width:100%;padding:13px 14px;background:transparent;border:none;border-radius:14px;color:#e5eefc;text-decoration:none;font-size:14px;font-weight:700;cursor:pointer;transition:background .2s,color .2s,transform .2s;text-align:left}'
 + '.menu-item:hover{background:rgba(255,255,255,0.08);color:#fff;transform:translateX(2px)}'
 + '.menu-item-icon{display:flex;align-items:center;justify-content:center;width:34px;height:34px;border-radius:12px;background:rgba(255,255,255,0.08);font-size:15px;flex-shrink:0}'
 + '.menu-item-text small{display:block;font-size:11px;font-weight:600;color:rgba(226,232,240,0.5);margin-top:2px}'
 + '.install-url-wrap{position:relative;z-index:1}'
 + '.install-note{display:flex;align-items:center;justify-content:space-between;gap:12px;font-size:12px;color:rgba(226,232,240,0.58);margin-top:12px;padding:0 2px}'
 + '.install-pill{display:inline-flex;align-items:center;gap:7px;padding:7px 11px;border-radius:999px;background:rgba(96,165,250,0.12);border:1px solid rgba(96,165,250,0.22);color:#bfdbfe;font-size:11px;font-weight:700;letter-spacing:.2px}'
 + '.toast{position:fixed;left:50%;bottom:20px;transform:translateX(-50%) translateY(14px);opacity:0;pointer-events:none;z-index:9999;max-width:92vw}'
 + '.toast.show{opacity:1;transform:translateX(-50%) translateY(0);transition:opacity .22s,transform .22s}'
 + '.toast-inner{display:flex;align-items:center;gap:10px;padding:12px 14px;border-radius:999px;background:rgba(15,23,42,0.92);border:1px solid rgba(255,255,255,0.14);box-shadow:0 18px 44px rgba(0,0,0,0.42);backdrop-filter:blur(22px);color:rgba(241,245,249,0.95);font-size:13px;font-weight:700}'
 + '.toast-dot{width:10px;height:10px;border-radius:999px;background:#60a5fa;box-shadow:0 0 0 4px rgba(96,165,250,0.18)}'
 + '.toast.ok .toast-dot{background:#34d399;box-shadow:0 0 0 4px rgba(52,211,153,0.16)}'
 + '.toast.err .toast-dot{background:#fb7185;box-shadow:0 0 0 4px rgba(251,113,133,0.16)}'
 + '.fab{position:fixed;right:16px;bottom:16px;z-index:9998;display:flex;flex-direction:column;gap:10px;align-items:flex-end}'
 + '.fab .fab-btn{display:flex;align-items:center;gap:10px;padding:12px 14px;border-radius:999px;background:rgba(255,255,255,0.12);border:1px solid rgba(255,255,255,0.16);backdrop-filter:blur(18px);box-shadow:0 14px 34px rgba(0,0,0,0.32);color:#fff;text-decoration:none;cursor:pointer;font-weight:800;font-size:13px;transition:transform .22s,background .22s,box-shadow .22s}'
 + '.fab .fab-btn:hover{transform:translateY(-2px);background:rgba(255,255,255,0.16);box-shadow:0 18px 38px rgba(0,0,0,0.34)}'
 + '.fab .fab-ico{width:30px;height:30px;border-radius:999px;background:rgba(15,23,42,0.45);border:1px solid rgba(255,255,255,0.12);display:flex;align-items:center;justify-content:center;font-size:14px}'
 + '.fab .fab-primary{background:rgba(59,130,246,0.22);border-color:rgba(59,130,246,0.25)}'
 + '.fab .fab-primary:hover{background:rgba(59,130,246,0.28)}'
 + '.fab .fab-success{background:rgba(16,185,129,0.18);border-color:rgba(16,185,129,0.22)}'
 + '.fab .fab-success:hover{background:rgba(16,185,129,0.24)}'
 ...
 + '@media(max-width:480px){.wrap{padding:0 4px}.card{padding:16px;border-radius:16px}.gen-btn{font-size:14px;padding:16px}.install-top{align-items:center}.install-title{font-size:20px}.hamburger-btn{width:44px;height:44px}.hamburger-menu{right:0;left:auto;min-width:210px}.fab{right:12px;bottom:12px}}'
...
+ '<p class="sub">Torrentio · jac.red · Knaben · Magnetz</p>'
 ...
 + '<div class="card install-shell"><div class="install-top"><div class="install-copy"><div class="install-kicker">Quick install</div><div class="install-title">Menu 3 gạch kiểu Facebook</div><div class="install-sub">Bấm biểu tượng 3 gạch để mở nhanh thao tác cài addon hoặc copy manifest URL, không cần kéo xuống tìm nút.</div></div><div class="menu-wrap" id="installMenu"><button class="hamburger-btn" id="menuBtn" onclick="toggleInstallMenu(event)" aria-label="Open install menu"><span></span><span></span><span></span></button><div class="hamburger-menu"><button class="menu-item" onclick="copyUrl();closeInstallMenu()"><span class="menu-item-icon">⧉</span><span class="menu-item-text">Copy URL<small>Sao chép manifest hiện tại</small></span></button><a class="menu-item" href="' + stremioUrl + '" id="slink" onclick="closeInstallMenu()"><span class="menu-item-icon">▶</span><span class="menu-item-text">Install<small>Mở Stremio để cài addon</small></span></a></div></div></div>'
 + '<div class="install-url-wrap"><div class="url-box" id="iurl" onclick="copyUrl()">' + installUrl + '</div><div class="install-note"><span>Chạm vào URL để copy nhanh hoặc dùng nút 3 gạch ở góc phải.</span><span class="install-pill">Manifest ready</span></div></div>'
 + '</div>'
 + '<div class="fab" id="fab">'
 + '<button class="fab-btn fab-primary" onclick="gen()"><span class="fab-ico">↻</span><span>Generate</span></button>'
 + '<button class="fab-btn" onclick="copyUrl()"><span class="fab-ico">⧉</span><span>Copy</span></button>'
 + '<a class="fab-btn fab-success" href="' + stremioUrl + '" id="fabInstall"><span class="fab-ico">▶</span><span>Install</span></a>'
 + '</div>'

 + '<div class="footer">Hybrid Addon v7.0.0 | <span>fatcatQN</span></div>'
 + '<div id="toast" class="toast" aria-live="polite" aria-atomic="true"><div class="toast-inner"><div class="toast-dot"></div><div class="toast-msg" id="toastMsg">OK</div></div></div>'
 + '</div>'
 ...
 + 'function closeInstallMenu(){var m=document.getElementById("installMenu");if(m)m.classList.remove("open")}'
 + 'function toggleInstallMenu(e){if(e)e.stopPropagation();var m=document.getElementById("installMenu");if(m)m.classList.toggle("open")}'
 + 'document.addEventListener("click",function(e){var m=document.getElementById("installMenu");if(!m)return;if(!m.contains(e.target))m.classList.remove("open")})'
 + 'var __toastTimer=null;'
 + 'function showToast(msg,type){var t=document.getElementById("toast");var m=document.getElementById("toastMsg");if(!t||!m)return;m.textContent=msg||"";t.classList.remove("ok","err");if(type==="err")t.classList.add("err");else if(type==="ok")t.classList.add("ok");t.classList.add("show");if(__toastTimer)clearTimeout(__toastTimer);__toastTimer=setTimeout(function(){t.classList.remove("show")},2200)}'
 + 'function gen(){var c=getCfg();var e=enc(c);var u=location.protocol+"//"+location.host+"/"+e+"/manifest.json";document.getElementById("iurl").textContent=u;document.getElementById("slink").href="stremio://"+u.replace(/^https?:\\/\\//,"");var fi=document.getElementById("fabInstall");if(fi)fi.href=document.getElementById("slink").href;closeInstallMenu();showToast("Đã cập nhật manifest", "ok")}'
 + 'function copyUrl(){var u=document.getElementById("iurl").textContent;if(navigator.clipboard){navigator.clipboard.writeText(u).then(function(){showToast("Đã copy URL", "ok")}).catch(function(){showToast("Copy thất bại", "err")})}else{prompt("Copy:",u)}}'
 ...
 + 'function applyTIO(){var l=document.getElementById("configLink").value.trim();if(!l){showToast("Dán link Torrentio trước đã", "err");return}var c=parseTIO(l);if(!c){showToast("Link Torrentio không hợp lệ", "err");return}tioCfg=c;gen();showToast("Đã áp dụng Torrentio", "ok")}'
